...
public class OntologyHelper {
    ...
    public static void main(String[] args){
    	...
    	//}
       	...
       	OWLClass atomicConcept=df.getOWLClass(IRI.create("#AtomicConcept"));
       	OWLObjectProperty hasModelingFeatures=df.getOWLObjectProperty
       			(IRI.create("#hasModelingFeatures"));
       	OWLClassExpression hasOnlyModelingFAll=df.getOWLObjectAllValuesFrom(hasModelingFeatures, atomicConcept);
       	
       	OWLClass alc=df.getOWLClass(IRI.create("#ALCCC"));
       	OWLSubClassOfAxiom subax=df.getOWLSubClassOfAxiom(alc, hasOnlyModelingFAll);
       	//manager.addAxiom(ontology, subax);
       	
       	Set<OWLClassExpression> arguments2=new HashSet<OWLClassExpression>();
       	arguments2.add(alc);
       	arguments2.add(hasOnlyModelingFAll);

       	OWLAxiom axiomaaa = df.getOWLEquivalentClassesAxiom(arguments2);
       	manager.addAxiom(ontology, axiomaaa);
       	Set<OWLClassExpression> argIntersection=new HashSet<OWLClassExpression>();
       	argIntersection.add(atomicConcept);
       	argIntersection.add(alc);
       	
       	OWLClassExpression oce=df.getOWLObjectUnionOf(argIntersection);
       	OWLAxiom axi = df.getOWLEquivalentClassesAxiom(oce);
       	manager.addAxiom(ontology, axi);
